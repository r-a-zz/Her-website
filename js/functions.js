...
async function fetchHoroscope(sign) {
	...
	resultBox.innerHTML = "Fetching your horoscope...";
	...
	try {
	  ...
	  resultBox.innerHTML = `
		<strong>${sign.toUpperCase()}</strong>: ${data.horoscope}
	  `;
	} catch (error) {
  ...
  async function searchMusic() {
	...
	try {
	  const response = await fetch(`https://www.googleapis.com/youtube/v3/search?part=snippet&q=${encodeURIComponent(query)}&key=YOUR_API_KEY&type=video&maxResults=1`);
	  const data = await response.json();
